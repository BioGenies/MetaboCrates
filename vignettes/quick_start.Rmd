...
Next, let's focus on the clearance of data. When ugrouped data passed, function below returns the names of metabolites which have more than the given treshold of missing values. If the grouping column is specified, then it returnes only metabolites for which the treshold is exceeded in each group level. In the examples below the treshold is 20%.
...
```{r}
create_PCA_plot(attr(comp_dat, "completed"))
```
...
```{r}
create_correlations_heatmap(attr(comp_dat, "completed")) +
...
```{r}
create_histogram(attr(comp_dat, "completed"), "C0")

create_boxplot(attr(comp_dat, "completed"), "C0")
 
create_qqplot(attr(comp_dat, "completed"), "C0")
```
...
Next tool allows for creating a barplot, showing the minimum number of the greatest variances explained by each principal component from a Principal Component Analysis, which cumulative sum is less or equal than given treshold. The plot also includes a line graph representing the cumulative variance explained by the components. Below, the treshold was set to 80%.
...
```{r}
create_PCA_plot(attr(comp_dat, "completed"), type = "group")
```
