...
params:
  ...
  corr_threshold_both: NULL
  corr_heatmap_metabolites_both: "all"
...
```{r, out.height="50%", out.width = "70%", eval=!is.null(attr(params[["dat"]], "group")), fig.cap="The counts of grouping levels.", fig.pos="H", fig.align='center'}
...
cat("The absolute correlation threshold for the heatmap of correlations before and after imputation was set to ",
    params[["corr_threshold_both"]]*100,
    "%. There were ",
    length(params[["corr_heatmap_metabolites_both"]]),
    " metabolites chosen to be visible: ",
    paste0(params[["corr_heatmap_metabolites_both"]], collapse = ", "),
    ".  \n", sep = "")
```

```{r, eval=!is.null(attr(params[["dat"]], "completed")), out.height="70%", out.width="80%", fig.cap="Heatmap of correlations before and after imputation (up to 10 chosen metabolites visible).", fig.align='center', fig.pos="H"}
cor_heatmap_both <- create_correlations_heatmap(
  params[["dat"]],
  threshold = ifelse(is.null(params[["corr_threshold_both"]]),
                     0,
                     params[["corr_threshold_both"]]),
  metabolites_to_display =
    params[["corr_heatmap_metabolites_both"]][1:min(length(params[["corr_heatmap_metabolites_both"]]), 10)],
  type = "both",
  interactive = FALSE
)

if(!is.null(cor_heatmap_both)){
  cor_heatmap_cap_both <- "Correlations heatmap (up to 10 chosen metabolites visible)."
  cor_heatmap_results_both <- "show"
}else{
  cor_heatmap_cap_both <- NULL
  cor_heatmap_results_both <- "asis"
}
```
```{r, eval=!is.null(attr(params[["dat"]], "completed")), out.height="50%", out.width="70%", fig.cap=cor_heatmap_cap_both, fig.align='center', fig.pos="H", results=ifelse(exists("cor_heatmap_results_both"), cor_heatmap_results_both, "show")}
if(is.null(cor_heatmap_cap_both)){
...
```{r, eval=!is.null(attr(params[["dat"]], "cv")), results="asis"}
cat("## Outlier detection  \n  \n")

cat("The absolute correlation threshold for the heatmap of correlations after imputation was set to ",
...
```{r, eval=!is.null(attr(params[["dat"]], "cv")), out.height="70%", out.width="80%", fig.cap="Heatmap of correlations after imputation (up to 10 chosen metabolites visible).", fig.align='center', fig.pos="H"}
...
```{r, eval=!is.null(attr(params[["dat"]], "cv")), out.height="50%", out.width="70%", fig.cap=cor_heatmap_cap, fig.align='center', fig.pos="H", results=ifelse(exists("cor_heatmap_results"), cor_heatmap_results, "show")}
...
```{r, results = "asis", eval=!is.null(attr(params[["dat"]], "cv"))}
cat("For the biplot based on the PCA including all sample types, the threshold
    was set to ",
...
```{r, eval=!is.null(attr(params[["dat"]], "cv")), out.height="80%", out.width="90%", fig.cap="Principal Component Analysis (PCA) including all sample types; (a) scatter plot with colors indicating sample types; (b) biplot  showing similarities among compounds based on their contribution to variability; (c) barplot showing the proprotion of variance explained by each principal component.", fig.align='center'}
...
ggarrange(list(pca_plt, biplot, pca_variance_plt), labels = "auto", ncol = 2)
```
...
```{r, eval=(!is.null(attr(params[["dat"]], "cv")) & !is.null(attr(params[["dat"]], "group"))), out.height="80%", out.width="90%", fig.cap="Principal Component Analysis (PCA) including only samples; (a) scatter plot with colors indicating group levels; (b) biplot  showing similarities among compounds based on their contribution to variability; (c) barplot shwoing the proprotion of variance explained by each principal component.", fig.align='center'}
pca_plt <- create_PCA_plot(
  params[["dat"]],
  group_by = "group",
  type = "scatterplot",
